...
export const getData = async (endPoint, token, hubId,) => {
  try {
    const res = await fetch(iaBaseApiUrl + "/" + endPoint + `?filters[hub][id][$eq]=${hubId}` + "&populate=*", {headers:{ Authorization: `Bearer ${token}` }} , {
      cache: "no-store",
    });
    if (!res?.ok) {
